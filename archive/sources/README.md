...
| `missing-media-continuation-2026-07-27.json` | ArchiveTeam, Internet Archive, MemGator, ImageShack, GitLab, Bitbucket e SourceForge | busca continuada: 22 URLs em 13 arquivos Memento configurados, 12 rotas Imagizer por ID, 57.647 arquivos extraídos do conjunto `4archive` e hospedagens públicas atribuíveis; nenhum payload candidato |
...
A continuação verificou as 17 revisões públicas da página ImageShack no ArchiveTeam, todas marcadas como não salvas, e não encontrou projeto público correspondente. Os dois mapas comprimidos do conjunto independente `4archive` foram baixados, tiveram os SHA-1 publicados pelo Internet Archive confirmados e expandiram para 57.647 arquivos e 196.339.103 bytes; a busca por nomes, IDs públicos e pares servidor/bucket retornou zero correspondências. O MemGator não relatou mementos para as 22 URLs exatas, mas houve falhas de acesso em pelo menos um arquivo para todas as consultas; por isso o resultado é registrado como busca agregada incompleta, não como negativa de cada acervo configurado.
